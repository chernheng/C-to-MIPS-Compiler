...
%token SEMI_COLON NAME NUMBER VAR_TYPE COMMA COLON HEX OP_PRODUCT_ASN OP_DIVIDE_ASN OP_MOD_ASN DOT DOUBLE FLOAT ESC_SEQ ONE_CHAR STRING

%type <string> NAME VAR_TYPE NUMBER HEX DOUBLE FLOAT ESC_SEQ ONE_CHAR STRING
...
FACTOR : VARIABLE     { $$ = $1; }    // variable
       ...
       | ONE_CHAR   { $$ = new OneCharacter($1); }
       | STRING     { $$ = new StringLiterals($1); }
       | HEX {$$ = new Number($1);}
       | DOUBLE {$$ = new Double($1);}
       | FLOAT {$$ = new Float($1);}
       | FUNCTION { $$ = $1;}
